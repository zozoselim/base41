...
def update_patient_requested_test(
    test_id: int,
    doctor_id: int,
    patient_id: int,
    test_name: str,
    ...
    with get_connection() as connection:
        existing = connection.execute(
            """
            SELECT id FROM patient_requested_tests
            WHERE id = ? AND doctor_id = ? AND patient_id = ?
            """,
            (test_id, doctor_id, patient_id),
        ).fetchone()
        if not existing:
            return None
        ...
        connection.execute(
            """
            UPDATE patient_requested_tests
            SET test_name = ?, test_date = ?, note = ?
            WHERE id = ? AND doctor_id = ? AND patient_id = ?
            """,
            (test_name.strip(), test_date.strip(), note.strip(), test_id, doctor_id, patient_id),
        )
        row = connection.execute(
            """
            SELECT ptr.*, d.name AS doctor_name
...
def delete_patient_requested_test(test_id: int, doctor_id: int, patient_id: int) -> bool:
    with get_connection() as connection:
        cursor = connection.execute(
            """
            DELETE FROM patient_requested_tests
            WHERE id = ? AND doctor_id = ? AND patient_id = ?
            """,
            (test_id, doctor_id, patient_id),
        )
