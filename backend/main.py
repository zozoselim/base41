...
class RequestedTestRequest(BaseModel):
    doctor_id: int
...
@app.post("/patients/{patient_id}/requested-tests")
def create_patient_requested_test(patient_id: int, payload: RequestedTestRequest) -> dict:
    if not get_patient(patient_id, doctor_id=payload.doctor_id):
        raise HTTPException(status_code=404, detail="Hasta bulunamadı")
    return save_patient_requested_test(
        payload.doctor_id,
        patient_id,
        payload.test_name,
        payload.test_date,
...
@app.put("/patients/{patient_id}/requested-tests/{test_id}")
def edit_patient_requested_test(patient_id: int, test_id: int, payload: RequestedTestRequest) -> dict:
    if not get_patient(patient_id, doctor_id=payload.doctor_id):
        raise HTTPException(status_code=404, detail="Hasta bulunamadı")
    updated = update_patient_requested_test(
        test_id,
        payload.doctor_id,
        patient_id,
        payload.test_name,
    ...
    if not updated:
        raise HTTPException(status_code=404, detail="Tetkik kaydı bulunamadı")
    return updated


@app.delete("/patients/{patient_id}/requested-tests/{test_id}")
def remove_patient_requested_test(patient_id: int, test_id: int, doctor_id: int) -> dict:
    if not get_patient(patient_id, doctor_id=doctor_id):
        raise HTTPException(status_code=404, detail="Hasta bulunamadı")
    deleted = delete_patient_requested_test(test_id, doctor_id, patient_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Tetkik kaydı bulunamadı")
...
def doctor_decision(payload: DoctorDecisionRequest) -> dict:
    ...
    if payload.decision == "approve":
        ...
        decision_note = decision_note or "İlaç doktor onayı ile hasta mevcut ilaç listesine eklendi."

    if payload.decision == "request_further_test":
        if not payload.test_name.strip() or not payload.test_date.strip():
            raise HTTPException(status_code=400, detail="Tetkik adı ve tarihi zorunludur")
